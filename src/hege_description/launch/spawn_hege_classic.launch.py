"""Bring up Gazebo CLASSIC with the Hege rover spawned in it.
    ...
    ros2 launch hege_description spawn_hege_classic.launch.py
    ros2 launch hege_description spawn_hege_classic.launch.py gui:=false

This is the original simulation, kept as it was. spawn_hege.launch.py is now
the Harmonic version and is the one to use; this file needs
ros-humble-gazebo-ros-pkgs and ros-humble-gazebo-ros2-control, which CANNOT be
installed alongside Gazebo Harmonic - the two conflict over /usr/bin/gz, and
installing one apt-removes the other. Since PX4 SITL requires Harmonic, a
machine set up for this project will not normally be able to run this file.

It is kept because it is the configuration every measured number in
nav2_params.yaml and dual_ekf_navsat.yaml came from, and because the Harmonic
port has not yet been validated against those numbers. See
docs/gazebo_harmonic.md.
"""
